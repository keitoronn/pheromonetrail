...
class antSys:
    ...
    def makerinsetu(self):
        mylist=[[self.params["nestPos"][0],self.params["nestPos"][1]]]
        dir=10000
        esatika=0
        dirbuf=[]
        kari=0
        ...
        esax=0
        esay=0
        for food in self.foodTS[-1]:
            if(esakyori>(food[0]*2+food[1]*2)):
                esakyori=food[0]*2+food[1]*2
                esax=food[0]
                esay=food[1]
        for ph in self.phTS[-1]:
            mylist.append([ph["pos"][0],ph["pos"][1]])
            dis=(ph["pos"][0]-esax)**2+(ph["pos"][1]-esay)**2
            if dis<dir:
                dir=dis
                esatika=kari+1
            kari += 1
        ...
        for ph in self.phTS[-1]:
            mylist.append([ph["pos"][0],ph["pos"][1]])
            for food in self.foodTS[-1]:
                dis=(ph["pos"][0]-food[0])**2+(ph["pos"][1]-food[1])**2
                if dis<dir and np.sqrt(dis)<self.params["pheromone_R"] and pkyo>food[0]**2+food[1]**2:
                    pkyo=food[0]**2+food[1]**2
                    dir=dis
                    esatika=kari+1
            kari += 1
    ...
    def aritrailstock(self):
        ...
        k=[np.array([0,0])]
        for ant in self.antTS:
            #print(ant)
            for i in range(len(ant)):
                antlist.append(ant[i])
                if(ant[i]["action"]== "gotFood"):
                    for j in self.antTS:
                        if(len(j)>i):
                            k.append(np.array([j[i]["pos"][0],j[i]["pos"][1]]))
                    #print(k)
                    ssk=0
                    ssk+=1
